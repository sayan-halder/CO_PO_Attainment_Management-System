...
public class CO2_Calculation_Details extends javax.swing.JFrame {
    MyConnection dbc=new MyConnection();
    Connection conn=dbc.connectme();
    ResultSet rs;
    PreparedStatement pstmt;
    
    public CO2_Calculation_Details() {
        super("CO2 CALCULATION DETAILS");
        initComponents();
    }
    ...
    public void jTable1()
    {
        try
        {
            ...
            pstmt=conn.prepareStatement("delete from CO2_CALCULATION");
            pstmt.execute();
            
            pstmt=conn.prepareStatement("insert into CO2_CALCULATION (USN, SUBJECT_CODE, QUESTION, TOTAL)\n" +
                                        "select USN, SUBJECT_CODE, sum(TCON)*8, sum(TM)*100\n" +
                                        "from CO\n" + 
                                        "group by USN, SUBJECT_CODE");
            pstmt.execute();
            
            pstmt=conn.prepareStatement("update CO2_CALCULATION set PERCENTAGE = TOTAL / QUESTION");
            pstmt.execute();
            
            pstmt=conn.prepareStatement("update CO2_CALCULATION set TARGET = 'Y' where PERCENTAGE >= 70.0");
            pstmt.execute();
            
            pstmt=conn.prepareStatement("select USN, TOTAL/100 as [Total Marks], PERCENTAGE, TARGET\n" +
                                        "from CO2_CALCULATION where SUBJECT_CODE=? order by USN");
    ...
    public void jTable2()
    {
        try
        {
            pstmt=conn.prepareStatement("delete from CO2_RESULT");
            pstmt.execute();

            pstmt=conn.prepareStatement("insert into CO2_RESULT (SUBJECT_CODE) values (?)");
            pstmt.setString(1,sub_code.getText());
            pstmt.execute();

            pstmt=conn.prepareStatement("update CO2_RESULT set TOTAL_STUDENTS=\n" +
                                        "select count(USN) from STUDENT");
            pstmt.execute();

            pstmt=conn.prepareStatement("update CO2_RESULT set TOTAL_CO=\n" +
                                        "select count(USN)*100 from CO2_CALCULATION\n" +
                                        "where TARGET='Y' and SUBJECT_CODE=?");
            pstmt.setString(1,sub_code.getText());
            pstmt.execute();
            
            pstmt=conn.prepareStatement("update CO2_RESULT set PERCENTAGE = TOTAL_CO / TOTAL_STUDENTS");
            pstmt.execute();

            pstmt=conn.prepareStatement("select TOTAL_STUDENTS, TOTAL_CO/100, PERCENTAGE\n" +
                                        "from CO2_RESULT where SUBJECT_CODE=?");
            pstmt.setString(1,sub_code.getText());
            rs=pstmt.executeQuery();
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(new javax.swing.border.LineBorder(new java.awt.Color(255, 255, 255), 5, true), "DETAILS OF CO2", javax.swing.border.TitledBorder.CENTER, javax.swing.border.TitledBorder.TOP, new java.awt.Font("Times New Roman", 1, 20), new java.awt.Color(255, 255, 255))); // NOI18N
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(CO2_Calculation_Details.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new CO2_Calculation_Details().setVisible(true);
            }
        });
    }
